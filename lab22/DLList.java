...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item)  {
        ...
        if (size == 0) {
            this.sentinel = new Node(item, null, null);
        ...
        while (pointer.next != sentinel) {
            if  (item.compareTo(pointer.item) < 0) {
                Node a =  new Node(item, pointer.prev, pointer);
                pointer.prev.next = a;
                pointer.prev = a;
            } if (pointer.next == sentinel) {
                Node a = new Node(item, pointer, this.sentinel);
                pointer.next = a;
            }
    ...
    public DLList<T> mergeSort() {
        ...
        int counter1 = 0;
        int counter2 = this.size / 2 + 1;
        Node pointer = sentinel.next;
        while (counter1 != this.size / 2) {
            oneHalf.addLast(pointer.item);
            pointer = pointer.next;
            counter1 += 1;
        }

        while (counter2 != this.size + 1) {
            otherHalf.addLast(pointer.item);
            pointer = pointer.next;
            counter2++;
        }
        DLList a = oneHalf.mergeSort();
        DLList b = otherHalf.mergeSort();

        DLList toReturn = b.merge(a);
        return toReturn;
    }
    ...
    public DLList<T> quicksort() {
        ...
        while (first != sentinel) {
            if (first.item.compareTo(pivot) < 0) {
                smallElements.addLast(first.item);
            } else if (first.item.compareTo(pivot) == 0) {
                equalElements.addLast(first.item);
            } else if (first.item.compareTo(pivot) > 0) {
                largeElements.addLast(first.item);
            }
            first = first.next;
        }

        DLList<T> small = smallElements.quicksort();
        DLList<T> equal = equalElements.quicksort();
        DLList<T> large = largeElements.quicksort();

        toReturn.append(small);
        toReturn.append(equal);
        toReturn.addLast(pivot);
        toReturn.append(large);
